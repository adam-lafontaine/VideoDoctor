...
namespace motion
{
    ...
    static void resize_up(SRC const& src, DST const& dst)
    {
        img::scale_up(src, dst);
        //img::resize(src, dst);
    }


    template <class SRC, class DST>
    static void resize_down(SRC const& src, DST const& dst)
    {
        img::scale_down(src, dst);
        //img::resize(src, dst);
    }
}
